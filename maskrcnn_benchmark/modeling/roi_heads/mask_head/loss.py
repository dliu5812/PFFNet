...
def project_masks_on_boxes(segmentation_masks, proposals, discretization_size, maskiou_on):
    ...
    for segmentation_mask, proposal in zip(segmentation_masks, proposals):
        ...
        if maskiou_on:
            ...
            x2 = int(proposal[2]) + 1
            y2 = int(proposal[3]) + 1
            # print('seg mask poly is', segmentation_mask)
            for poly_ in segmentation_mask.polygons:
                # print('each poly', poly_)
...
class MaskRCNNLossComputation(object):
    ...
    def match_proposals_to_targets(self, proposal, target):
        match_quality_matrix = boxlist_iou(target, proposal)
        matched_idxs = self.proposal_matcher(match_quality_matrix)
        # Mask RCNN needs "labels" and "masks "fields for creating the targets
        target = target.copy_with_fields(["labels", "masks"])
    ...
    def prepare_targets(self, proposals, targets):
        ...
        masks = []
        mask_ratios = []
        # print('num of proposals, ', proposals)
        # print('num of gt, ', targets)
    ...
    def __call__(self, proposals, mask_logits, targets):
        ...
        """
        # print('proposals, tgt', proposals, targets)
        ...
        if self.maskiou_on:
            ...
            mask_sum_area = pred_masks.sum(dim=[1, 2]) + mask_targets_full_area
            mask_sum_area = torch.max(mask_sum_area, value_1)
            maskdice_targets = 2 * mask_ovr_area / mask_sum_area

            quality_alpha = 0.5

            mask_quality_targets = quality_alpha * maskiou_targets + (1-quality_alpha) * maskdice_targets
        ...
        if not self.maskiou_on:
            return mask_loss, selected_mask, labels, None
        else:
            return mask_loss, selected_mask, labels, mask_quality_targets
